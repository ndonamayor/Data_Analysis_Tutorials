---
title: "Data analysis with R: AUDIT & reversal learning RT"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating AUDIT scores with reaction time in a probabilistic reversal learning task of both patients and healthy controls
...
Fantastic, let's start with some info about the AUDIT and probabilistic reversal task!
...
my_data <- all_data %>% 
  select(group, subject, gender, age, AUDIT = auditSum, RT = reversalRTmedian) %>%
  drop_na(AUDIT, RT)
...
Obviously, to identify implausible values in your dataset, you first need to know the possible range of AUDIT scores (hint: check the table on [page 22](https://www.who.int/publications/i/item/WHO-MSD-MSB-01.6a) of the manual). You also need to know that reaction times below 100 ms are usually discarded in cognitive tasks.
...
my_data <- my_data %>% 
  ...
  mutate(z.AUDIT = scale(AUDIT), z.RT = scale(RT)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between AUDIT scores and the reaction times in the reversal learning task.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total AUDIT scores and the reaction times in the probabilistic reversal learning task correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between alcohol use severity and learning performance?

If total AUDIT scores and reaction times were strongly positively correlated (e.g. *r* = .81), could you conclude that severe alcohol use slows learning from reinforcement?

In the session on tasks, we talked about typical outcome measures from the probabilistic reversal learning task. What other outcome measures could you correlate with AUDIT scores?
...
That's it, you analyzed real AUDIT and probabilistic reversal data using a correlation!
