---
title: "Data analysis with R: AUDIT & number of achieved reversals"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating AUDIT scores with the number of reversals in a probabilistic reversal learning task of both patients and healthy controls
...
my_data <- all_data %>% 
  select(group, subject, gender, age, AUDIT = auditSum, number_reversals = reversalNum) %>%
  drop_na(AUDIT, number_reversals)
...
Obviously, to identify implausible values in your dataset, you first need to know the possible range of AUDIT scores (hint: check the table on [page 22](hhttps://www.who.int/publications/i/item/WHO-MSD-MSB-01.6a) of the manual). You also need to know the maximum number of achievable reversals in the probabilistic reversal task, which was 20.
...
my_data <- my_data %>%
  filter(number_reversals %in% (0:20) & AUDIT %in% (0:40))
...
my_data <- my_data %>% 
  ...
  mutate(z.AUDIT = scale(AUDIT), z.number_reversals = scale(number_reversals)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
  ...
  select(-c(z.AUDIT, z.number_reversals))
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between AUDIT scores and the number of achieved reversals in the reversal learning task.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total AUDIT scores and the number of achieved reversal stages correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between alcohol use severity and sensitivity to reinforcement?

If total AUDIT scores and the number of achieved reversal stages were strongly negatively correlated (e.g. *r* = -.96), could you conclude that severe alcohol use impairs sensitivity to reinforcement?
