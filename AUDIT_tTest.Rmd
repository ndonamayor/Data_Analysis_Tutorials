---
title: "Data analysis with R: AUDIT"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the average AUDIT scores of patients and healthy controls
...
Specifically, you will compare the **scores in the Alcohol Use Disorders Identification Test (AUDIT)** (auditSum) of a group of psychiatric patients to those of a group of healthy controls. You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
Fantastic, let's start with some info about the AUDIT!
...
my_data <- all_data %>% 
  select(group, subject, gender, age, AUDIT = auditSum) %>%
  drop_na(AUDIT)
...
my_data %>% 
  ...
  summarize(______ = mean(AUDIT), ______ = sd(AUDIT), 
            ______ = min(AUDIT), ______ = max(AUDIT))
...
my_data <- my_data %>% 
  ...
  mutate(AUDIT = ifelse(abs(z) > 3, NA, AUDIT)) %>% 
...
You are now ready to compare the AUDIT scores of patients and controls. 
...
What conclusions can you draw from the results of your t-test about the severity of alcohol use in your groups?

Based on the results of your analysis, would you predict a similar result in another sample of healthy participants vs. patients with some particular psychiatric disorder (other than alcohol dependency)?
...
That's it, you analyzed real AUDIT data using a t-test!
