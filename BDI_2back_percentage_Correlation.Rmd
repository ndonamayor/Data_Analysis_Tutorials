---
title: "Data analysis with R: BDI-II & 2-back hit percentage"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating BDI-II scores with the percentage of hits in the 2-back condition of both patients and healthy controls
...
Fantastic, let's start with some info about the BDI-II and N-back task!
...
my_data <- all_data %>% 
  select(group, subject, gender, age, BDI = bdiSum, hits_2back = X2backHitPerc) %>%
  drop_na(BDI, hits_2back)
...
my_data <- my_data %>% 
  filter(hits_2back %in% (0:100) & BDI %in% (0:63))
...
my_data <- my_data %>% 
  ...
  mutate(z.BDI = scale(BDI), z.hits_2back = scale(hits_2back)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between BDI-II scores and the percentage of hits in the 2-back condition.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total BDI-II scores and the percentage of hits in the 2-back condition correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between depression severity and working memory performance?

If total BDI-II scores and the percentage of hits in the 2-back condition were strongly negatively correlated (e.g. *r* = -.72), could you conclude that severe depression impairs working memory?
...
That's it, you analyzed real BDI-II and 2-back data using a correlation!
