---
title: "Data analysis with R: BDI-II & reversal learning RT"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating BDI-II scores with reaction time in a probabilistic reversal learning task of both patients and healthy controls
...
Fantastic, let's start with some info about the BDI-II and probabilistic reversal task!
...
my_data <- all_data %>%
  select(group, subject, gender, age, BDI = bdiSum, RT = reversalRTmedian) %>%
  drop_na(BDI, RT)
...
my_data %>% 
  ...
  summarize(mean.______ = mean(BDI), sd.______ = sd(BDI), 
            min.______ = min(BDI), max.______ = max(BDI),
...
Obviously, to identify implausible values in your dataset, you first need to know the possible range of BDI-II total scores (hint: just multiply the [minimum/maximum possible score per question](https://naviauxlab.ucsd.edu/wp-content/uploads/2020/09/BDI21.pdf) by the number of questions). You also need to know that reaction times below 100 ms are usually discarded in cognitive tasks.
...
my_data <- my_data %>%
  ...
  mutate(z.BDI = scale(BDI), z.RT = scale(RT)) %>%
  # "ungroup" the dataset
  ungroup() %>%
  ...
  mutate(BDI = ifelse(abs(z.BDI) > 3, NA, BDI),
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between BDI-II scores and the reaction times in the reversal learning task.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total BDI-II scores and the reaction times in the probabilistic reversal learning task correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between depression severity and learning performance?

If total BDI-II scores and reaction times were strongly positively correlated (e.g. *r* = .77), could you conclude that severe depression slows learning from reinforcement?

In the session on tasks, we talked about typical outcome measures from the probabilistic reversal learning task. What other outcome measures could you correlate with BDI-II scores?
...
That's it, you analyzed real BDI-II and probabilistic reversal data using a correlation!
