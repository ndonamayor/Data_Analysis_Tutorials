---
title: "Data analysis with R: BDI-II & number of achieved reversals"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating BDI-II scores with the number of reversals in a probabilistic reversal learning task of both patients and healthy controls
...
my_data <- all_data %>% 
  select(group, subject, gender, age, BDI = bdiSum, number_reversals = reversalNum) %>%
  drop_na(BDI, number_reversals)
...
Obviously, to identify implausible values in your dataset, you first need to know the possible range of BDI-II total scores (hint: just multiply the [minimum/maximum possible score per question](https://naviauxlab.ucsd.edu/wp-content/uploads/2020/09/BDI21.pdf) by the number of questions). You also need to know the maximum number of achievable reversals in the probabilistic reversal task, which was 20.
...
my_data <- my_data %>%
  filter(number_reversals %in% (0:20) & BDI %in% (0:63))
...
my_data <- my_data %>% 
  ...
  mutate(z.BDI = scale(BDI), z.number_reversals = scale(number_reversals)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
  ...
  select(-c(z.BDI, z.number_reversals))
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between BDI-II scores and the number of achieved reversals in the reversal learning task.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total BDI-II scores and the number of achieved reversal stages correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between depression severity and sensitivity to reinforcement?

If total BDI-II scores and the number of achieved reversal stages were strongly negatively correlated (e.g. *r* = -.89), could you conclude that severe depression impairs sensitivity to reinforcement?
