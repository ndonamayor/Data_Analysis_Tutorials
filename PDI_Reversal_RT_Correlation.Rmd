---
title: "Data analysis with R: PDI & reversal learning RT"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating PDI scores with reaction time in a probabilistic reversal learning task of both patients and healthy controls
...
Specifically, you will correlate the **total scores in the Peters' Delusions Inventory (PDI)** (pdiSum) with the **average reaction times** (reversalRTmedian) in a probabilistic reversal learning task. This correlation will use the data from both psychiatric patients and healthy controls. You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
Fantastic, let's start with some info about the PDI and probabilistic reversal task!


### What is the Peters' Delusions Inventory?

The [**PDI**](https://academic.oup.com/schizophreniabulletin/article/30/4/1005/1930847) is a 21-item questionnaire designed to measure delusional ideation, such as delusion-like or hallucinatory experiences. It is suitable for the assessment of both clinical and subsyndromal samples. 

In each of the 21 items, the participant is first asked whether they have experienced a certain thought or feeling (e.g. *Do you ever feel as if people seem to drop hints about you or say things with a double meaning?*). In case of an affirmative response, the participant is asked to rate to what extent they find the thought distressing, how often they think about it and how strongly they believe it is true, using 5-point scales. Higher PDI scores indicate higher levels of delusional ideation.

The PDI has been translated to numerous languages, including German, and has good psychometric properties, i.e. high  [reliability](https://dictionary.apa.org/reliability) and [validity](https://dictionary.apa.org/validity).


### What is a probabilistic reversal task?

**Probabilistic reversal learning tasks** (e.g. [here](https://www.jneurosci.org/content/jneuro/22/11/4563) or [here](https://www.jneurosci.org/content/36/43/10935)) are used to assess sensitivity to [reinforcement](https://dictionary.apa.org/reinforcement). Participants are generally shown two stimuli simultaneously, one of which is rewarded with a higher probability than the other, and are asked to find out which stimulus is better through trial-and-error. After a certain number of trials, the reward probability of the stimuli is reversed (i.e. the previously better one is now rewarded with the lower probability). 

The occurrence of reversals can be determined in two main ways. As in [this version](https://www.jneurosci.org/content/36/43/10935), it can take place after a predetermined number of trials, irrespective of participant performance. If the task is adaptive, as in the version you used or in [this one](https://www.jneurosci.org/content/jneuro/22/11/4563), reversals take place after the participant has made a certain number of consecutive correct choices (i.e. chosen the high reward stimulus). Therefore, in adaptive tasks, the number of achieved reversals depends on how well the participant performed the task.

```{r fig3, echo = FALSE, out.width = "45%", fig.align = "center", fig.cap = "Example of a probabilistic reversal learning task (figure from Waltmann et al. Sufficient reliability of the behavioral and computational readouts of a probabilistic reversal learning task. *Behav Res* 54; 2022: 2993–3014. doi: [10.3758/s13428-021-01739-7](https://doi.org/10.3758/s13428-021-01739-7), licensed under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/))."}

knitr::include_graphics("https://media.springernature.com/full/springer-static/image/art%3A10.3758%2Fs13428-021-01739-7/MediaObjects/13428_2021_1739_Fig1_HTML.png")
...
How do you think reaction times relate to the participants' ability to learn from positive and negative feedback, if at all?
...
Obviously, in this module, we took care of experimental design and recruitment for you. Still, you chose to correlate PDI scores and reaction times in the probabilistic reversal learning task, so you probably have some idea about why this could be interesting. Before you dive into data analysis, take a few minutes to consider what results you would expect, based on common sense and the clinical knowledge you acquired in this module.
...
my_data <- all_data %>% 
  select(group, subject, gender, age, PDI = pdiSum, RT = reversalRTmedian) %>%
  drop_na(PDI, RT)
...
Why would you choose the individual participant's reaction time median instead of the arithmetic mean?
...
my_data %>% 
  ...
  summarize(mean.______ = mean(PDI), sd.______ = sd(PDI), 
            min.______ = min(PDI), max.______ = max(PDI),
            mean.______ = mean(RT), sd.______ = sd(RT), 
            min.______ = min(RT), max.______ = max(RT))
...
Obviously, to identify implausible values in your PDI variable, you first need to know the possible range of the PDI's grand total score (hint: check the first paragraph of [page 1009](https://academic.oup.com/schizophreniabulletin/article/30/4/1005/1930847)). You also need to know that reaction times below 100 ms are usually discarded in cognitive tasks.
...
Why do you think that very fast reaction times (< 100 ms) are usually discarded in **cognitive** tasks?
...
There are some reaction times below 100 ms. Before dealing with any potential outliers, you need to get rid of these cases, which would otherwise distort your *z*-scores.

<div class = "exercise_instruction">

You are just going to filter the data, in order to only keep subjects with reaction times of 100 ms or above. Then have R print out the filtered `my_data` data frame.
...
question(
  ...
  answer("Yes, in the control group"),
  answer("Yes, in both groups", correct = TRUE),
  answer("No"),
...
my_data <- my_data %>% 
  ...
  mutate(z.PDI = scale(PDI), z.RT = scale(RT)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
  ...
  mutate(PDI = ifelse(abs(z.PDI) > 3, NA, PDI),
         RT = ifelse(abs(z.RT) > 3, NA, RT)) %>% 
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between PDI scores and the reaction times in the reversal learning task.
...
What do the descriptive statistics of the PDI scores tell you about delusional ideation in your groups?
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total PDI scores and the reaction times in the probabilistic reversal learning task correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between delusional ideation and learning performance?

If total PDI scores and reaction times were strongly positively correlated (e.g. *r* = .85), could you conclude that high delusional ideation slows learning from reinforcement?

In the session on tasks, we talked about typical outcome measures from the probabilistic reversal learning task. What other outcome measures could you correlate with PDI scores?
...
ggplot(data = my_data, aes(x = ______, y = ______, color = group)) +
  ...
  geom_jitter(width = .3, height = 0, size = 3, alpha = .4) +
...
That's it, you analyzed real PDI and probabilistic reversal data using a correlation!
