---
title: "Data analysis with R: PDI & number of achieved reversals"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating PDI scores with the number of reversals in a probabilistic reversal learning task of both patients and healthy controls
...
What do you think high or low numbers of achieved reversals indicate in our adaptive probabilistic reversal task?
...
my_data <- all_data %>% 
  select(group, subject, gender, age, PDI = pdiSum, number_reversals = reversalNum) %>%
  drop_na(PDI, number_reversals)
...
my_data %>% 
  ...
  summarize(mean.______ = mean(PDI), sd.______ = sd(PDI), 
            ...
            mean.______ = mean(number_reversals), sd.______ = sd(number_reversals), 
            min.______ = min(number_reversals), max.______ = max(number_reversals))
...
Obviously, to identify implausible values in your PDI variable, you first need to know the possible range of the PDI's grand total score (hint: check the first paragraph of [page 1009](https://academic.oup.com/schizophreniabulletin/article/30/4/1005/1930847)). You also need to know the maximum number of achievable reversals in the probabilistic reversal task, which was 20.
...
my_data <- my_data %>%
  filter(number_reversals %in% (0:20) & PDI %in% (0:336))
...
my_data <- my_data %>% 
  ...
  mutate(z.PDI = scale(PDI), z.number_reversals = scale(number_reversals)) %>% 
  # "ungroup" the dataset
  ungroup() %>% 
  ...
  mutate(PDI = ifelse(abs(z.PDI) > 3, NA, PDI),
         number_reversals = ifelse(abs(z.number_reversals) > 3, NA, number_reversals)) %>% 
  ...
  select(-c(z.PDI, z.number_reversals))
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between PDI scores and the number of achieved reversals in the reversal learning task.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are total PDI scores and the number of achieved reversal stages correlated?

In what direction are your data correlated and what conclusions can you draw from your results about the relationship between delusional ideation and sensitivity to reinforcement?

If total PDI scores and the number of achieved reversal stages were strongly negatively correlated (e.g. *r* = -.90), could you conclude that high delusional ideation impairs sensitivity to reinforcement?
...
ggplot(data = my_data, aes(x = ______, y = ______, color = group)) +
  ...
  geom_jitter(width = .3, height = .3, size = 3, alpha = .4) +
