---
title: "Data analysis with R: PDI"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the average PDI scores of patients and healthy controls
...
Specifically, you will compare the **total scores in the Peters' Delusions Inventory (PDI)** (pdiSum) of a group of psychiatric patients to those of a group of healthy controls. You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
Fantastic, let's start with some info about the PDI!
...
my_data <- all_data %>% 
  select(group, subject, gender, age, PDI = pdiSum) %>%
  drop_na(PDI)
...
my_data %>% 
  ...
  summarize(______ = mean(PDI), ______ = sd(PDI), 
            ______ = min(PDI), ______ = max(PDI))
...
Obviously, to identify implausible values in your dataset, you first need to know the possible range of the PDI's grand total score (hint: check the first paragraph of [page 1009](https://academic.oup.com/schizophreniabulletin/article/30/4/1005/1930847)). 
...
my_data <- my_data %>% 
  ...
  mutate(PDI = ifelse(abs(z) > 3, NA, PDI)) %>% 
...
You are now ready to compare the PDI scores of patients and controls. 
...
What conclusions can you draw from the results of your t-test about the delusional ideation of your groups?

Based on the results of your analysis, would you predict a similar result in another sample of healthy participants vs. patients with schizophrenia?
...
That's it, you analyzed real PDI data using a t-test!
