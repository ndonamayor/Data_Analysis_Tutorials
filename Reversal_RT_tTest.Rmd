---
title: "Data analysis with R: Reversal learning RT"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the reaction times of patients and healthy controls in a probabilistic reversal learning task
...
Specifically, you will compare the **average reaction times** (reversalRTmedian) in a probabilistic reversal learning task of a group of psychiatric patients to those of a group of healthy controls. You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
Fantastic, let's start with some info about the probabilistic reversal task!
...
Obviously, in this module, we took care of experimental design and recruitment for you. Still, you chose to analyze the reaction times in the probabilistic reversal learning task in the patient vs. control sample, so you probably have some idea about why this could be interesting. Before you dive into data analysis, take a few minutes to consider what results you would expect, based on common sense and the clinical knowledge you acquired in this module.
...
my_data <- all_data %>% 
  select(group, subject, gender, age, RT = reversalRTmedian) %>%
  drop_na(RT)
...
my_data %>% 
  ...
  summarize(______ = mean(RT), ______ = sd(RT), 
            ______= min(RT), ______ = max(RT))
...
Obviously, to identify implausible values in your variable, you first need to know that reaction times below 100 ms are usually discarded in cognitive tasks.
...
my_data <- my_data %>% 
  ...
  mutate(RT = ifelse(abs(z) > 3, NA, RT)) %>% 
...
You are now ready to compare the reaction times of patients and controls in the probabilistic learning task. 
...
What conclusions can you draw from the results of your t-test about the learning performance of your groups?
...
That's it, you analyzed real probabilistic reversal data using a t-test!
