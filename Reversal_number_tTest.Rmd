---
title: "Data analysis with R: Number of achieved reversals"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the number of achieved reversals of patients and healthy controls
...
my_data <- all_data %>% 
  select(group, subject, gender, age, number_reversals = reversalNum) %>%
  drop_na(number_reversals)
...
my_data %>% 
  ...
  summarize(______ = mean(number_reversals), ______ = sd(number_reversals), 
            ______ = min(number_reversals), ______ = max(number_reversals))
...
Obviously, to identify implausible values in your variable, you first need to know the maximum number of achievable reversals in the probabilistic reversal task, which was 20.
...
my_data <- my_data %>%
  filter(number_reversals %in% (0:20))
...
It seems that your data is pretty clean, since you have neither implausible values nor outliers. So you can proceed directly to the analysis!
...
my_data <- my_data %>% 
  group_by(group) %>% 
  mutate(z = scale(number_reversals)) %>% 
  ungroup() %>% 
  mutate(number_reversals = ifelse(abs(z) > 3, NA, number_reversals)) %>% 
  drop_na(number_reversals) %>% 
  select(-z)
...
You are now ready to compare the number of reversals achieved by patients and controls in the probabilistic learning task. 
...
What conclusions can you draw from the results of your t-test about the sensitivity to reinforcement of your groups?
