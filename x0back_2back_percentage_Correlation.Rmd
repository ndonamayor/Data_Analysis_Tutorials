---
title: "Data analysis with R: 0-back & 2-back hit percentage"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Relating the percentage of hits in the 0-back and 2-back conditions of both patients and healthy controls
...
Specifically, you will correlate the **percentage of hits in the 0-back condition** (X0backHitPerc) with that of the **2-back condition** (X2backHitPerc) of an N-back task. The percentage of hits is calculated based on the ratio between correct responses (hits) and [false alarms](https://dictionary.apa.org/false-alarm). This correlation will use the data from both psychiatric patients and healthy controls. You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
Fantastic, let's start with some info about the N-back task!
...
How much working memory involvement would you expect in the 0-back condition?

You might want to consider whether the 0-back condition may assess any other [cognitive function](https://link.springer.com/referenceworkentry/10.1007%2F978-94-007-0753-5_426).

Compared to the 0-back condition, do you think the 2-back condition is easier or harder? 
...
Obviously, in this module, we took care of experimental design and recruitment for you. Still, you chose to correlate the percentage of hits in the 0-back and 2-back conditions, so you probably have some idea about why this could be interesting. Before you dive into data analysis, take a few minutes to consider what results you would expect, based on common sense and the clinical knowledge you acquired in this module.
...
my_data <- all_data %>% 
  select(group, subject, gender, age, hits_0back = X0backHitPerc, hits_2back = X2backHitPerc) %>%
  drop_na(hits_0back, hits_2back)
...
my_data %>% 
  ...
  summarize(mean.______ = mean(hits_0back), sd.______ = sd(hits_0back), 
            min.______ = min(hits_0back), max.______ = max(hits_0back),
...
my_data <- my_data %>% 
  filter(hits_0back <= 100 & hits_2back <= 100)
...
my_data <- my_data %>% 
  filter(hits_0back %in% (0:100) & hits_2back %in% (0:100))
...
my_data <- my_data %>%
  ...
  mutate(z.hits_0back = scale(hits_0back), z.hits_2back = scale(hits_2back)) %>%
  # "ungroup" the dataset
  ungroup() %>%
  ...
  mutate(hits_0back = ifelse(abs(z.hits_0back) > 3, NA, hits_0back),
  ...
  select(-c(z.hits_0back, z.hits_2back))
...
You are now going to analyze whether there is a statistical relationship (which might be causal or not) between the percentage of hits in the 0-back and 2-back conditions.
...
How [strongly](https://www.dummies.com/education/math/statistics/how-to-interpret-a-correlation-coefficient-r/) are the percentage of hits in the 0-back and 2-back conditions correlated?

In what direction are your data correlated and what conclusions can you draw from your results about working memory performance in the N-back task?

If the percentage of hits in the 0-back and 2-back conditions were strongly positively correlated (e.g. *r* = .94), could you conclude that poor 0-back performance impairs 2-back performance?
...
# put the 0-back data in the x-axis
ggplot(data = my_data, aes(x = ______, y = ______, color = group)) +
  ...
  geom_jitter(width = 2, height = 2, size = 3, alpha = .4) +
...
That's it, you analyzed real N-back data using a correlation!
