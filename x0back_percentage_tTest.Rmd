---
title: "Data analysis with R: 0-back hit percentage"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the percentage of hits in the 0-back condition of patients and healthy controls
...
Specifically, you will compare the **percentage of hits in the 0-back condition** (X0backHitPerc) of an [N-back task](https://www.psytoolkit.org/experiment-library/nback.html) of a group of psychiatric patients to those of a group of healthy controls. The percentage of hits is calculated based on the ratio between correct responses (hits) and [false alarms](https://dictionary.apa.org/false-alarm). You will go through all necessary steps to end up with a meaningful output that you can report in a scientific way.
...
With regard to other conditions in the N-back task, do you think the 0-back condition is easier or harder? 
...
Obviously, in this module, we took care of experimental design and recruitment for you. Still, you chose to analyze the percentage of hits in the 0-back condition in the patient vs. control sample, so you probably have some idea about why this could be interesting. Before you dive into data analysis, take a few minutes to consider what results you would expect, based on common sense and the clinical knowledge you acquired in this module.
...
my_data <- all_data %>% 
  select(group, subject, gender, age, hits_0back = X0backHitPerc) %>%
  drop_na(hits_0back)
...
my_data %>% 
  ...
  summarize(______= mean(hits_0back), ______ = sd(hits_0back), 
            ______= min(hits_0back), ______ = max(hits_0back))
...
my_data <- my_data %>%
  ...
  mutate(hits_0back = ifelse(abs(z) > 3, NA, hits_0back)) %>%
...
You are now ready to compare the percentage of hits of patients and controls in the 0-back condition. 
...
What conclusions can you draw from the results of your t-test about the cognitive capacity of your groups?

Looking at the patient's mean percentage of hits in the 0-back condition, how would you expect patients' performance to look in the 2-back condition?
...
That's it, you analyzed real 0-back data using a t-test!
