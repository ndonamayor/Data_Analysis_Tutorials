---
title: "Data analysis with R: 2-back hit percentage"
output: 
  learnr::tutorial:
    progressive: TRUE
    allow_skip: TRUE
...
### Comparing the percentage of hits in the 2-back condition of patients and healthy controls
...
Obviously, in this module, we took care of experimental design and recruitment for you. Still, you chose to analyze the percentage of hits in the 2-back condition in the patient vs. control sample, so you probably have some idea about why this could be interesting. Before you dive into data analysis, take a few minutes to consider what results you would expect, based on common sense and the clinical knowledge you acquired in this module.
...
my_data <- all_data %>% 
  select(group, subject, gender, age, hits_2back = X2backHitPerc) %>%
  drop_na(hits_2back)
...
my_data %>% 
  ...
  summarize(______ = mean(hits_2back), ______ = sd(hits_2back), 
            ______ = min(hits_2back), ______ = max(hits_2back))
...
my_data <- my_data %>% 
  ...
  mutate(hits_2back = ifelse(abs(z) > 3, NA, hits_2back)) %>% 
...
You are now ready to compare the percentage of hits of patients and controls in the 2-back condition. 
